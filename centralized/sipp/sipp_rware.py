...
class SippPlanner(SippGraph):
    def __init__(self, param, agent_id):
        SippGraph.__init__(self, param)
        self.start = tuple(param["agents"][agent_id]["start"])
        self.goal = tuple(param["agents"][agent_id]["goal"])
        self.name = param["agents"][agent_id]["name"]
        self.open = []
...
def main():
    ...
    # compute first plan
    sipp_planner = SippPlanner(param, 0)
